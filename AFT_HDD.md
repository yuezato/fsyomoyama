...
HDDの仕事としては
1. 136KiB書き込み最後の4KiB書き込みをやるために余分に1セクタ読まなければならないが
2. 次の136KiB書き込みが見えているので、余分に読んだ1セクタに次の136KiBの先頭2KiBを合流させる

ということの繰り返しになり、1回の書き込みで1セクタ読み込みが発生するという償却的な見方ができます。
こういう解釈をすると、 4096位置で512バイト書くパターンと同じかなあと思います。

その一方で、**4KiB+132KiB**の書き込みでグチャグチャになってしまう理由は良く分かりません。
この辺はもうHDDのinternalな話だからなあ……

# XFSのベンチで不思議なことになることの解説
書く
