...
これで遅くなる理由は、今回使っているHDDが512eというものになっているからだと考えられます。

この辺が参考になるかも:　https://github.com/yuezato/fsyomoyama/blob/master/AFT_HDD.md

すなわち
* 新規に追加していく場合は4KiBアライメントが意識された書き込みになっている。
* 一度作られたファイルで行う場合には4KiBアライメントが意識されず、書き込み開始位置などが中途半端になり、かえっておそくなる。
  * 余分な512バイト書き込みは確かに行われなくなるが、それ以上に遅くなる。
